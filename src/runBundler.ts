...
import shappire from '@oasisprotocol/sapphire-paratime';
...
export async function runBundler (argv: string[], overrideExit = true): Promise<BundlerServer> {
  ...
  let wallet: Wallet
  try {
    if(config.network == "https://testnet.sapphire.oasis.dev/") {
      mnemonic = fs.readFileSync(config.mnemonic, 'ascii')
      wallet = shappire.wrap(Wallet.fromMnemonic(mnemonic)).connect(provider);
    } else {
      mnemonic = fs.readFileSync(config.mnemonic, 'ascii')
